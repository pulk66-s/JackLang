...
namespace IO
{
    ...
    void Logger::ast(std::string fmt, ...)
    {
        if (Logger::hasType(Logger::Type::AST)) {
            File f("log/ast.json");
            va_list args, copy;
            va_start(args, fmt);
            va_copy(copy, args);
            std::cout << YELLOW << "[AST] " << NC;
            vprintf(fmt.c_str(), args);
            f.append_vargs(fmt, copy);
            va_end(args);
        }
    ...
    void Logger::ast_debug(std::string fmt, ...)
    {
        if (Logger::hasType(Logger::Type::AST_DEBUG)) {
            File f("log/ast.log");
            va_list args, copy;
            va_start(args, fmt);
            va_copy(copy, args);
            std::cout << CYAN << "[AST_DEBUG] " << NC;
            vprintf(fmt.c_str(), args);
            f.append_vargs(fmt, copy);
            va_end(args);
        }
